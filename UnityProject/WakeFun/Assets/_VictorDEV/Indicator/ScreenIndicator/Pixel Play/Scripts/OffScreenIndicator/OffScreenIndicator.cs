...
using UnityEngine;
using VictorDev.IndicatorUtils.ScreenIndicator;
...
public class OffScreenIndicator : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        screenCentre = new Vector3(Screen.width, Screen.height, 0) * 0.5f;
    ...
    void DrawIndicators()
    {
        foreach (IndicatorTarget target in targets)
        {
            ...
            if (isTargetVisible)
            {
                ...
                indicator.gameObject.SetActive(target.NeedBoxIndicator);
            }
            else if (!isTargetVisible)
            {
                ...
                indicator.gameObject.SetActive(target.NeedArrowIndicator);
            }
            if (indicator)
            {
                ...
                //功能擴充 by Victor
                indicator.extended_Indicator.Setting(target.extended_Target);
            }
        }
    }
